...
#----------------------------------------------------------------------以下为账单模块--------------------------------------------------------------------------
    ...
    def tongHangZhangDan(self): #同行账单
        try:
            ...
            assert u"同行所属网点" in self.driver.page_source  # 断言“同行所属网点”是否在页面源码中
            #设置变量chaXun
    ...
    def yeWuGuiZeSheZhi(self):   #业务规则设置
        try:
            ...
            self.driver.find_element_by_id('checkboxState').click() #选择 启用
            time.sleep(1)
            self.driver.find_element_by_partial_link_text(u'保存').click()    #点击 保存
            time.sleep(2)
